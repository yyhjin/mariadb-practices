...
-- 2) select절에 그룹함수(통계함수) 있는 경우, 어떤 컬럼도 select절에 올 수 없다!!
-- 	  emp_no는 아무런 의미가 없다.
-- 	  오류!!!
...
-- 4) group by에 참여 컬럼은 projection이 가능하다: select절에 올 수 있다.
-- 예제) 사원별 평균 연봉
...
--    이미 where절은 실행이 되었기 떄문에 having절에서 조건을 주어야 한다.
-- 예제) 평균 연봉이 6000달러 이상인 사원의 사번과 평균 연봉을 출력
select emp_no, avg(salary)
from salaries
group by emp_no
